...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		...
		for (Tweet element : tweets) {
			if (element.getAuthor().toLowerCase() == username.toLowerCase())
				Result.add(element);
		}
		
		/*
		System.out.println(Result.size());
		for(Tweet eleTweet : Result) {
			System.out.println(eleTweet.getAuthor());
		}
		*/
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		...
		List<Tweet> Result = new ArrayList<Tweet>();
		for (Tweet element : tweets) {
			if (element.getTimestamp().isBefore(timespan.getEnd())
					&& element.getTimestamp().isAfter(timespan.getStart()))
				Result.add(element);
		}
		return Result;
	}
